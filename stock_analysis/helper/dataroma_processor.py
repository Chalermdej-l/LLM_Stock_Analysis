...
class DataromaScraper:
    ...
    def scrape_insider_buy_data(self, path_url):
        ...
        df_insider_buy = (
            df_insider_buy.groupby(groupby_columns)
            .agg({"amount": "sum", "shares": "sum", "count": "sum"})
            .reset_index()
        )
    ...
    def scrape_home_data(self):
        ...
        df_insider_buy_home = pd.DataFrame(
            self.scrape_table(tables[0]), columns=["date_filling", "company", "total_value", "price"]
        )
        ...
        df_insider_buy_home["date_filling"] = df_insider_buy_home["date_filling"].str.replace(
            "Today", datetime.date.today().strftime("%d %b")
        )
        df_insider_buy_home["date_filling"] = (
            df_insider_buy_home["date_filling"] + " " + datetime.date.today().strftime("%Y")
        )
        ...
        df_insider_buy_home = df_insider_buy_home.groupby(groupby_columns).agg({"total_value": "sum"}).reset_index()
